/**
 * Split a built system prompt into semantic items and estimate their size.
 * No pi access.
 *
 * Splitting relies on structural markers pi's buildSystemPrompt() emits
 * deterministically:
 *
 ...
 * - the base prompt ends before the "Current working directory" footer, so
...
/** The slice of BuildSystemPromptOptions measurement needs. */
export interface PromptOptionsSlice {
	cwd: string;
	/** Home directory used to abbreviate context-file paths. */
	homeDir?: string;
	customPrompt?: string;
...
/** One active tool as it contributes to the context. */
export interface ToolSlice {
	name: string;
	/** Sent to the provider with every request. */
	description: string;
	/** JSON-serialized parameter schema; sent with every request. */
	...
	guidelines: string[];
	/** Provenance, e.g. "builtin" or "npm:pi-web-access". */
	source: string;
}
...
/**
 * Split a system prompt into pi's base prompt, appended prompt, context files,
 * skills, active tool contributions and the aggregate appended by extensions.
 */
...
/**
 * Measure tool contributions: per-tool definition payloads plus the prompt
 * snippet and guideline lines carved out of the base prompt. Built-in tools
 * collapse into one aggregate pi-native item.
 */
function measureTools(base: string, tools: ToolSlice[], items: InjectionItem[], carvedSpans: Span[]): void {
	...
	for (const tool of tools) {
		...
		const source: InjectionSource = { id: `tool-source:${tool.source}`, label: tool.source, native: false };
		items.push(
			createItem(`tool:${tool.source}:${tool.name}`, "tool", source, tool.name, promptText + definition),
		);
	}
	if (builtinChildren.length > 0) {
		...
		items.push(
			createAggregateItem(
				...
				"tool",
				PI_SOURCE,
				`Built-in Tools (${builtinChildren.length})`,
				builtinChildren,
			),
		);
	}
...
/** Build an item with derived token size. */
function createItem(
	id: string,
	kind: InjectionKind,
	...
	return { id, kind, source, label, tokens: textTokens(text), text };
}

/** Build an aggregate whose total exactly reconciles with its child items. */
...
/**
 * Locate pi's dynamic cwd footer so it can be excluded from the base prompt and
 * from extension additions. The cwd line must match the exact resolved cwd on a
 * complete line, so ordinary prompt text mentioning the cwd is not mistaken for
 * the footer.
 */
...
/** Span of the first exact occurrence of needle. */
...
/** One context file's content without its project-instructions wrapper. */

/**
 * Context-usage classification: combine the prompt/tool decomposition with the
 * live session messages into estimated category totals.
 */
...
/**
 * Estimate the composition of the next request. Prompt and tool categories come
 * from the snapshot, message categories from the live session context. Empty
 * categories are dropped and every aggregate equals the sum of its children.
 */
 ...
 * when every entry is message-backed.
 */
...
/** Map snapshot items to prompt/tool/memory/skill categories. */
function classifyPromptCategories(snapshot: InjectionSnapshot): UsageCategory[] {
		...
		for (const item of group.items) {
			switch (item.kind) {
				...
				case "skills":
					...
					break;
				case "message":
					// Session-backed messages are counted by classifyMessages.
					break;
			}
		}
	}
...
/** Classify live session messages and frozen context-only injections. */
function classifyMessages(
	...
	const toolOutput = aggregate(
		...
		withoutEmpty([
			...
			leaf("bash-executions", "Bash", bashExecutions),
		]),
	);
	return withoutEmpty([
 ...
 * estimateTokens alone cannot see is counted. Undefined for messages the
 * transform excludes from the context.
 */
...
/**
 * Thinking previews, accounted once per assistant message. Provider-reported
 * reasoning replaces a smaller visible-text estimate; signature-only estimates
 * stay approximate metadata.
 */
function thinkingEntries(message: AssistantContextMessage): UsagePreviewEntry[] {
	...
	if (first !== undefined) {
		entries[0] = {
			...first,
			tokens: first.tokens + (countedTokens - visibleTokens),
...
/** Describe a positive invisible share or signature-size proxy without retaining its bytes. */
function createInvisibleReasoningEstimate(
	...
	if (reportedTokens !== undefined) {
		...
		return tokens === 0 ? undefined : { tokens, basis: "provider-reported", encoded: signatureChars > 0 };
	}
	if (signatureChars === 0) return undefined;
	// chars/4 over a signature is a rough proxy, not a bound: the server decrypts
	// the envelope and counts the reconstructed thinking at a model-dependent ratio.
	return { tokens: textTokens(signatureChars), basis: "signature-proxy", encoded: true };
}
...
/** String length of an optional signature field without exposing its contents. */
...
/** Parent category whose total is the exact sum of its children. */
...
function withoutEmpty(categories: Array<UsageCategory | undefined>): UsageCategory[] {
	return categories.filter(
		(category): category is UsageCategory => category !== undefined && category.tokens > 0,
	);
}
...
/** The chars/4 heuristic pi's estimateTokens uses for text content. */

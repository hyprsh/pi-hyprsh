...
| **Context** | `/context` shows what occupies the model context as a proportional map; `/context injections` shows the hidden parts — base prompt, tool definitions, skills, memory files, extension additions — as a previewable tree. `↑↓`/`jk` to move, `Enter` to preview, `Z` to zoom the map, `Esc`/`q` to close. Once anything has been delegated, a line above the map attributes it: how many units ran, how much context they carried elsewhere and how much of it came back here. Those tokens are outside the map on purpose — the cells are proportions of this window and delegated reading was never in it. |
